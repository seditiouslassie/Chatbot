...
START, LOCALITY, CITY, PINCODE, EMAIL, MODEOFCONTACT, REQ, BOARD, STANDARD, SUBJECTS, DEAL, CONFIRM, MOREDETAILS, END = range(14)
...
def locality(update, context):
    ...
    if(re.search(regex,context.user_data['Locality'])):
        ...
        update.message.reply_text('Please enter a valid Locality Name',reply_markup=ReplyKeyboardRemove())
        context.user_data['Locality'] = update.message.text
        user = update.message.from_user
        return LOCALITY
...
def city(update, context):
    ...
    if(re.search(regex,context.user_data['City'])):
        ...
        reply_markup=ReplyKeyboardRemove())
         ...
         update.message.reply_text('Please enter a valid City Name.',reply_markup=ReplyKeyboardRemove())
         user = update.message.from_user
         context.user_data['City'] = update.message.text
         return CITY
...
def confirm(update, context):
    ...
    reply_keyboard = [['Yes', 'No']]
    update.message.reply_text(f'''{context.user_data['Deal']} : 
...
def moredetails(update, context):
	#if user says yes in previous confirm function 
	reply_keyboard = [['Yes' , 'No']]
	update.message.reply_text(f''' {context.user_data['Confirm']} :
		...
		reply_markup=KeyboardMarkup(Reply_keyboard, one_time_keyboard=True))
	user = update.message.from_user
	#user = update.message.from_user
	context.user.data['Confirm'] = update.message.text
	if update.message.text == "Yes" :
		return START 
		...
		update.message.reply_text(
				...
				Thankyou for going green! 
				We will find a match and get back to you soon. 
    ...
    conv_handler = ConversationHandler(
        ...
        states={
           START:[MessageHandler(Filters.text, start)],
            ...
            MOREDETAILS: [MessageHandler(Filters.text, confirm)], 
